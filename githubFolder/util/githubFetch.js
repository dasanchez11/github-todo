export class GithubFetch {
  constructor() {
    ...
    this.authToken = "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";
  }

  getUsers = async (searchQuery, page = 1, per_page = 5) => {
    const pagination = `&page=${page}&per_page=${per_page}`;
    const response = await fetch(
      this.baseUrl + `search/users?q=${searchQuery}` + pagination
    );
  ...
  getRepositories = async (searchUser, page = 1, per_page = 15) => {
    ...
    const res = await fetch(
      this.baseUrl + `users/${searchUser}/repos` + "?q=" + pagination,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
